...
with tab2:
    ...
    st.write("Mode ini digunakan untuk **Perencanaan Proyek Baru**. Masukkan target kapasitas yang ingin dicapai, dan sistem akan menghitung dimensi tanggul yang ideal untuk Anda berdasarkan batasan ketersediaan lahan atau tinggi dinding.")
    
    st.markdown("---")
    
    # Input Parameter Wajib
    col_d1, col_d2 = st.columns(2)
    target_kapasitas_desain = col_d1.number_input("Target Kapasitas 100% (KL / m³)", min_value=1.0, value=1000.0)
    tebal_dinding_asumsi = col_d2.number_input("Rencana Ketebalan Dinding (m)", min_value=0.1, value=0.3)
    ...
    mode_desain = st.radio(
        ...
        ["Kunci Luas Tanah (Sistem akan mencari Tinggi Dinding yang tepat)", 
         "Kunci Tinggi Dinding (Sistem akan mencari Luas Tanah yang tepat)"]
    )
    
    st.markdown("<br>", unsafe_allow_html=True)
    
    if mode_desain == "Kunci Luas Tanah (Sistem akan mencari Tinggi Dinding yang tepat)":
        st.info("Gunakan mode ini jika lahan di kilang sudah dipatok batasnya dan tidak bisa diperlebar.")
        cd1, cd2 = st.columns(2)
        p_lahan = cd1.number_input("Batas Panjang Lahan (m)", min_value=1.0, value=30.0)
        l_lahan = cd2.number_input("Batas Lebar Lahan (m)", min_value=1.0, value=30.0)
        
        if st.button("🏗️ GENERATE TINGGI DINDING", type="primary", use_container_width=True):
            # Rumus: Volume = Tinggi * (Panjang Dalam) * (Lebar Dalam)
            panjang_dalam = p_lahan - (2 * tebal_dinding_asumsi)
            lebar_dalam = l_lahan - (2 * tebal_dinding_asumsi)
            luas_dalam = panjang_dalam * lebar_dalam
            
            if luas_dalam <= 0:
                st.error("Luas lahan terlalu sempit untuk ketebalan dinding yang direncanakan!")
            else:
                tinggi_req = target_kapasitas_desain / luas_dalam
                ...
                if tinggi_req <= 1.8:
                    ...
                    st.warning("⚠️ **Peringatan NFPA 30:** Tinggi ini melebihi batas standar (1.8m) yang menyulitkan evakuasi & sirkulasi udara. Silakan perluas ukuran lahan Anda.")
                    
    else:
        st.info("Gunakan mode ini jika Anda punya tanah yang sangat luas, tapi ingin dindingnya tetap pendek agar murah & mudah inspeksinya.")
        ...
        if st.button("📐 GENERATE LUAS LAHAN", type="primary", use_container_width=True):
            # Rumus: Luas Dalam = Volume / Tinggi
            luas_dalam_req = target_kapasitas_desain / t_dinding_plan
            
            # Asumsi kita buat persegi sempurna (Sisi x Sisi)
            sisi_dalam = math.sqrt(luas_dalam_req)
            
            # Tambah ketebalan dinding luar
            sisi_luar = sisi_dalam + (2 * tebal_dinding_asumsi)
            
            st.markdown("### 📊 Hasil Rekomendasi Desain:")
            st.success(f"✅ Untuk dinding setinggi **{t_dinding_plan} m**, Anda membutuhkan lahan kosong (Area Footprint) berukuran:")
            st.markdown(f"#### **{sisi_luar:.2f} m x {sisi_luar:.2f} m**")
            st.write(f"*(Luasan ini sudah memperhitungkan ketebalan dinding {tebal_dinding_asumsi} m di setiap sisinya).*")
